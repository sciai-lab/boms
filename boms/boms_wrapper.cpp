...
py::array meanshift_cpp(py::array coords_np, py::array genes_np, int n_genes, int k, int max_iter, float h_s, float h_r, int use_flows, py::array flows_np, float alpha, int verbose) {
    float* coords;
    float* genes;
    int N, dim_s, dim_r;
    ...
    float* modes = meanshift(coords, genes, N, dim_s, n_genes, k, max_iter, h_s, h_r, 1, 0, 1, flows, height, width, use_flows, alpha, verbose);
